...
router.post("/", async (req, res) => {
  ...
  const takenNames = new Set(((await supabaseAdmin.from("agents").select("name").eq("user_id", userId)).data || []).map((a) => a.name.toLowerCase()));
  let finalName = name; let nameSource = "user";
  if (!finalName) { finalName = await generateAgentName({ role, goal, taken: takenNames }); nameSource = "brain"; }
  else if (takenNames.has(finalName.toLowerCase())) return res.status(409).json({ error: `You already have an agent named "${finalName}". Pick another name.` });
  ...
  const character = suppliedCharacter && typeof suppliedCharacter === "object" ? {
    ...generatedCharacter, ...suppliedCharacter,
  ...
  const system_prompt = [
    ...
    "", "PROFESSIONAL IDENTITY TAGS — these describe what you are and what you specialize in, not your tools:", tags.join(", "), "",
    "CHARACTER — this is a persistent part of your identity, not a temporary role-play:", characterPrompt(character), "",
  ...
  const mergedTools = [...new Set([...BUILTIN_TOOLS, ...(Array.isArray(allowed_plugins) ? allowed_plugins : [])])];
  const { data, error } = await supabaseAdmin.from("agents").insert({ user_id: userId, name: finalName, name_source: nameSource, role, goal, system_prompt, character, tags, allowed_plugins: mergedTools }).select().single();
  if (error) {
...
router.patch("/:id", async (req, res) => {
  ...
  if (Array.isArray(updates.allowed_plugins)) updates.allowed_plugins = [...new Set([...BUILTIN_TOOLS, ...updates.allowed_plugins])];
  ...
  if (updates.character && typeof updates.character === "object") {
    ...
    if (current) updates.system_prompt = [`You are ${current.name}, an AI agent whose job is: ${current.goal || current.role}.`, "Act on behalf of your user, use only the tools you've been given access to, and always pause for approval before sending, deleting, paying, or publishing anything.", "", "PROFESSIONAL IDENTITY TAGS — these describe what you are and what you specialize in, not your tools:", Array.isArray(current.tags) ? current.tags.join(", ") : "", "", "CHARACTER — this is a persistent part of your identity:", characterPrompt(updates.character), "", "Stay consistent with this character across the conversation."].join("\n");
    }
  }
  const { data, error } = await supabaseAdmin.from("agents").update(updates).eq("id", id).eq("user_id", userId).select().single();
  if (error) { if (error.code === "23505") return res.status(409).json({ error: "That agent name is already taken." }); return res.status(500).json({ error: error.message }); }
  res.json({ agent: data });
});
