...
import { buildAgentCharacter } from "../lib/character.js";
...
// POST /api/agents  { name?, role, goal, allowed_plugins?, character? }
router.post("/", async (req, res) => {
  ...
  // Every agent gets a persistent character. The Brain/UI may supply a richer
  // character, but the server always guarantees a useful default.
  const character = suppliedCharacter && typeof suppliedCharacter === "object"
    ? { ...buildAgentCharacter({ role, goal }), ...suppliedCharacter, version: 1 }
    : buildAgentCharacter({ role, goal });
...
router.patch("/:id", async (req, res) => {
  ...
  const { id } = req.params;
  const allowedFields = [
    "name", "role", "goal", "system_prompt", "character", "allowed_plugins",
