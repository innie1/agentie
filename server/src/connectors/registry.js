...
export const CONNECTORS = {
  gmail: { id: "gmail", name: "Gmail", actions: { searchEmails: { name: "searchEmails", irreversible: false, description: "Search emails" }, readThread: { name: "readThread", irreversible: false, description: "Read an email thread" }, draftEmail: { name: "draftEmail", irreversible: false, description: "Create an email draft" }, sendEmail: { name: "sendEmail", irreversible: true, description: "Send an email" } } },
  gcal: { id: "gcal", name: "Google Calendar", actions: { checkAvailability: { name: "checkAvailability", irreversible: false, description: "Check calendar events" }, createMeeting: { name: "createMeeting", irreversible: true, description: "Create a calendar event" } } },
  slack: { id: "slack", name: "Slack", actions: { readChannel: { name: "readChannel", irreversible: false, description: "Read channel history" }, sendMessage: { name: "sendMessage", irreversible: true, description: "Send a Slack message" } } },
  github: { id: "github", name: "GitHub", actions: { searchRepos: { name: "searchRepos", irreversible: false, description: "Search repositories" }, readIssue: { name: "readIssue", irreversible: false, description: "Read an issue or pull request" }, createIssue: { name: "createIssue", irreversible: false, description: "Create an issue" }, mergePullRequest: { name: "mergePullRequest", irreversible: true, description: "Merge a pull request" } } },
  notion: { id: "notion", name: "Notion", actions: { searchPages: { name: "searchPages", irreversible: false, description: "Search Notion" }, readDoc: { name: "readDoc", irreversible: false, description: "Read a Notion page" }, createPage: { name: "createPage", irreversible: false, description: "Create a Notion page" }, deletePage: { name: "deletePage", irreversible: true, description: "Delete a Notion page" } } },
  gdrive: { id: "gdrive", name: "Google Drive", actions: { searchFiles: { name: "searchFiles", irreversible: false, description: "Search Google Drive" }, getFile: { name: "getFile", irreversible: false, description: "Read Google Drive file metadata or content" }, createFile: { name: "createFile", irreversible: false, description: "Create a Google Drive file" }, updateFile: { name: "updateFile", irreversible: false, description: "Update Google Drive file metadata" }, deleteFile: { name: "deleteFile", irreversible: true, description: "Delete a Google Drive file" } } },
  canva: { id: "canva", name: "Canva", actions: { searchDesigns: { name: "searchDesigns", irreversible: false, description: "Search the user's Canva designs" }, getDesign: { name: "getDesign", irreversible: false, description: "Get Canva design metadata and links" }, createDesign: { name: "createDesign", irreversible: false, description: "Create a Canva design" }, listBrandTemplates: { name: "listBrandTemplates", irreversible: false, description: "Search accessible Canva brand templates" }, getBrandTemplate: { name: "getBrandTemplate", irreversible: false, description: "Get Canva brand template metadata" } } },
  whatsapp: { id: "whatsapp", name: "WhatsApp Business", actions: { sendMessage: { name: "sendMessage", irreversible: true, description: "Send a WhatsApp message" }, getBusinessProfile: { name: "getBusinessProfile", irreversible: false, description: "Read WhatsApp business profile" } } },
  stripe: { id: "stripe", name: "Stripe", actions: { getInvoice: { name: "getInvoice", irreversible: false, description: "Inspect a Stripe invoice" }, createInvoice: { name: "createInvoice", irreversible: false, description: "Create a Stripe draft invoice" }, chargeCustomer: { name: "chargeCustomer", irreversible: true, description: "Create a Stripe payment intent" } } },
  shopify: { id: "shopify", name: "Shopify", actions: { getShop: { name: "getShop", irreversible: false, description: "Read Shopify shop information" }, searchProducts: { name: "searchProducts", irreversible: false, description: "Search Shopify products" }, searchOrders: { name: "searchOrders", irreversible: false, description: "Search Shopify orders" }, createProduct: { name: "createProduct", irreversible: false, description: "Create a Shopify product" } } }
};

function parseCredentialValues(credentials) {
  const source = credentials?.values && typeof credentials.values === "object" ? credentials.values : credentials;
  const result = {};
  if (!source || typeof source !== "object") return result;
  for (const [key, value] of Object.entries(source)) {
    if (value === null || value === undefined) continue;
    if (typeof value === "string") {
      try { result[key] = decrypt(value); } catch { result[key] = value; }
    } else result[key] = value;
  }
  return result;
}

async function connectionFor(userId, connectorId) {
  const { data, error } = await supabaseAdmin.from("user_plugins").select("credentials,status").eq("user_id", userId).eq("plugin_id", connectorId).maybeSingle();
  ...
  const credentials = parseCredentialValues(data.credentials);
  if (!Object.keys(credentials).length) throw new Error(`${connectorId} has no usable credentials configured`);
  return credentials;
}
...
export async function runAction(connectorId, actionName, params = {}, context = {}) {
  ...
  const credentials = await connectionFor(context.userId, connectorId);
  let data;
  const token = credentials.access_token || credentials.accessToken || credentials.token || credentials.api_key || credentials.apiKey || credentials.key;
  if (["whatsapp","stripe","shopify"].includes(connectorId)) {
    if (connectorId === "whatsapp") data = await whatsapp(actionName, params, credentials);
    if (connectorId === "stripe") data = await stripe(actionName, params, credentials);
    if (connectorId === "shopify") data = await shopify(actionName, params, credentials);
  } else {
    if (!token) throw new Error(`${connectorId} has no access token in its connected credentials`);
    switch (connectorId) {
